...
VIDEO_STYLE_SUFFIXES = {'.mp4', '.mpg', '.avi'}
IMAGE_STYLE_SUFFIXES = {'.jpg', '.jpeg', '.png'}
...
def build_output_dir(base_output, style_path, preserve_color):
    style_suffix = style_path.suffix.lower()
    parts = ['videos']

    if style_suffix in VIDEO_STYLE_SUFFIXES:
        parts.append('from_video_style')
    elif style_suffix in IMAGE_STYLE_SUFFIXES:
        parts.append('from_image_style')
    else:
        raise ValueError(f'Unsupported style file type: {style_path.suffix}')
...
parser.add_argument('--output', type=str, default='output',
                    help='Base directory to save output videos by mode')
...
output_dir = build_output_dir(args.output, style_path, args.preserve_color)
...
if style_suffix in VIDEO_STYLE_SUFFIXES:
    ...
    assert style_video_length == content_video_length, (
